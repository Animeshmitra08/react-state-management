...
const Register = () => {
    ...
    async function insertdata(e) {
        e.preventDefault();
        try {            
            const res = await fetch("https://localhost:7279/api/Employee",{
                method : "POST",
  ...
  return (
    ...
    <section className=''>
        ...
        <form method='POST' onSubmit={insertdata} className="max-w-xl mx-auto bg-slate-100 mt-[4em] mb-[2em] p-6 rounded-lg shadow-md">
            ...
            <div className="mb-5">
                ...
                <select id="countries" name='describeYou' onChange={handleChange} value={formData.describeYou} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg cursor-pointer focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                ...
                <div className="flex">
                    {
                        recommendFriend.map((d, id)=>(
                            <div key={id} className="flex items-center me-4">
                                <fieldset id={id} defaultValue={formData.recommendFriend}>
                                    <input id={id} type="radio" value={d} onChange={handleChange} name="recommendFriend" className="w-4 h-4  cursor-pointer text-blue-600 bg-gray-100 border-gray-300 "/>
                ...
                <div className='grid w-full gap-6 md:grid-cols-3'>
                    {
                        frameworks.map((item)=>(                            
                        <div key={item.id}>
                            <input type="checkbox" id={item.forId} name={item.name} onChange={handleCheckboxChange} className="hidden peer" required=""/>
                            <label htmlFor={item.forId} className="inline-flex items-center justify-between w-full p-3 text-gray-500 bg-slate-50 border-2 border-gray-300 rounded-lg cursor-pointer  peer-checked:border-blue-600 hover:text-gray-600 peer-checked:text-gray-600 hover:bg-gray-100">                           
                                <div className="block"> 
                                    <img className='w-7 h-7 mb-2' src={item.imgLink} alt="react"/> 
                                    <div className="text-sm font-medium">{item.name}</div>
                                    <div className="text-xs">{item.definition}</div>
                                </div>
                            </label>
                        </div>
                        ))
            ...
            <div className="mb-5">
                ...
                <textarea id="message" rows="4" name='suggestions' value={formData.suggestions} onChange={handleChange} className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 " placeholder="Write your suggestions here..."></textarea>
            </div>
